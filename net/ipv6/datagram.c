			   ...
			   int addr_len)
{
	...
	/* ILNP Codes: Migrated from linux 3.9.7 by ry6. Originally from dp32 */
	sk->is_ilnp = 2; 	//mark that connect() is called
	if (is_ilnp6(daddr)) {
	  pr_debug("[ILNP]: Connect to ILNP host\n");
	  sk->is_ilnp = 1;
	}
	else pr_debug("[ILNP]: Connect to non-ILNP host\n");

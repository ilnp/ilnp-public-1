...
/* [ILNP] add by dp32, we cheat here to reduce metric every time new default route is added
 * - migrated by ry6 */
int metric_for_ilnp = 1024;
...
static struct rt6_info *ip6_route_info_create(struct fib6_config *cfg)
{
	...
	if (addr_type & IPV6_ADDR_MULTICAST)
		...
		pr_debug("[route.c] ip6_route_info_create(): cfg->fc_flags & RTF_LOCAL\n");
				     ...
				     unsigned int pref)
{
	struct fib6_config cfg = {
		...
		.fc_metric	= metric_for_ilnp, // [ILNP] TODL: use to be IP6_RT_PRIO_USER, (hack by dp32 - migrated by ry6)
		.fc_ifindex	= dev->ifindex,

...
static void icmpv6_echo_reply(struct sk_buff *skb)
{
	...
	/*[ILNP] Check if it is ILNP session*/
	pr_debug("[ILNP] echo_reply sk->is_ilnp == %u, skb->is_ilnp -- %u \n",sk->is_ilnp, skb->is_ilnp);
	if (sk->is_ilnp == 1 || skb->is_ilnp == 1) {
		pr_debug("[ILNP] Receive ping from ILNP host, replying...");
		sk->is_ilnp = 1; // ILNP - ry6 - Hack to force ilnp on new Sock
		pr_debug("[ILNP] echo_reply sk->is_ilnp == %u, skb->is_ilnp -- %u \n",sk->is_ilnp, skb->is_ilnp);
	}
...
/* This method is added by dp32 to send ILNPv6 Locator Update (LU) message (modified from ndisc_send_skb)
 * Originally written for Linux 3.9.7 migrated to Linux 4.9.40 by ry6.
 */

void icmpv6_send_lu(struct sk_buff *skb,
		    ...
		    const struct in6_addr *saddr)
{
	...
	dst = icmpv6_route_lookup(net, skb, sk, &fl6);
	if (IS_ERR(dst)) {
		pr_warning("[ILNP] icmpv6_route_lookup error\n");
		goto out;
	}

				 ...
				 struct netdev_queue *txq)
{
	...
	if (unlikely(test_bit(__QDISC_STATE_DEACTIVATED, &q->state))) {
		...
		rc = NET_XMIT_DROP;
		pr_debug("[dev.c] __dev_xmit_skb(): rc = NET_XMIT_DROP\n");
		...
		rc = NET_XMIT_SUCCESS;
		pr_debug("[dev.c] __dev_xmit_skb(): rc = NET_XMIT_SUCCESS\n");
...
static int __dev_queue_xmit(struct sk_buff *skb, void *accel_priv)
{
	...
	struct Qdisc *q;
	int rc = -ENOMEM;
	pr_debug("[dev.c] __dev_queue_xmit(): \n");
	skb_reset_mac_header(skb);
...
# ifdef CONFIG_NET_EGRESS
	if (static_key_false(&egress_needed)) {
		...
		if (!skb)
			goto out;
		pr_debug("[dev.c] __dev_queue_xmit(): (skb = sch_handle_egress) != null, goto out: not called; \n");
	}
# endif
#endif
	...
	if (dev->priv_flags & IFF_XMIT_DST_RELEASE){
		pr_debug("[dev.c] __dev_queue_xmit(): about to do skb_dst_drop(skb)\n");
		skb_dst_drop(skb);
	}
	else
		skb_dst_force(skb);

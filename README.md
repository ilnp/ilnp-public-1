...
1. A modifictaion to `glibc` ([.deb here - ilnp-glibc-1](https://github.com/ilnp/ilnp-glibc-1/ilnp-glibc-1.deb)) so that names for nodes in `/etc/hosts` can be mapped to ILNP Identifier-Locator Vector (I-LV) values, and passed up to applications in `struct addrinfo` via calls to `getaddrinfo(3)`.
2. Extensions to the Linux kernel v4.9 LTS ([.deb file here - ilnp-public-1](https://github.com/ilnp/ilnp-public-1/ilnp-public-1.deb)) to implement the ILNP functionality.
